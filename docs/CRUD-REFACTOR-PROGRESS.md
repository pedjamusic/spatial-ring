...
All models share the same dynamic route pattern under `/admin`:
...
| `/admin/assets/new` | Create asset |
| `/admin/assets/cmlfphr2m…/edit` | Edit asset |
| `/admin/assetCategories` | Asset category list |
| `/admin/events` | Event list |

**List page query params** (synced via `useSearchParams()`):

| Param | Default | Example |
|-------|---------|---------|
| `search` | _(empty)_ | `?search=guitar` |
| `page` | `1` | `?page=3` |
| `limit` | `25` | `?limit=50` |

Combined: `/admin/assets?search=guitar&page=2&limit=50`

IDs are Prisma CUIDs (e.g. `cmlfphr2m0004xdwqhj2puwx6`), generated by `@default(cuid())` in the schema.
